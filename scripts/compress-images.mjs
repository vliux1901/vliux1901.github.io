...
const outputRoot = path.join(root, 'tmp/compressed_images');
...
async function main() {
  ...
  for (const source of files) {
    ...
    // Retain the input extension to avoid collisions between photo.jpg and photo.png.
    const destination = path.join(outputRoot, `${relative}.webp`);
    await mkdir(path.dirname(destination), { recursive: true });
    const scratch = await mkdtemp(path.join(path.dirname(destination), '.compress-'));
    try {
      const temporary = path.join(scratch, 'output.webp');
